...
prizes = rawData
  ...
  .map((line) => {
    ...
    x = Number(x.split("=")[1]);
    y = Number(y.split("=")[1]);
    [ax, ay] = a.split(", ").map((s) => Number(s.split("+")[1]));
    [bx, by] = b.split(", ").map((s) => Number(s.split("+")[1]));
    return {
      a: { x: ax, y: ay },
      b: { x: bx, y: by },
      p: { x, y },
...
function getWinnable(prize) {
  let { a, b, p } = prize;
  bMax = Math.max(...[b.x, b.y]);
  bi = (a.x * p.y - a.y * p.x) / (a.x * b.y - a.y * b.x);
  ai = (p.x - bi * b.x) / a.x;
  if (isInt(ai) && isInt(bi)) {
    return { a: Math.abs(ai), b: Math.abs(bi) };
  }
...
tokens = 0;
for (prize of prizes) {
  let { a: ai, b: bi } = getWinnable(prize);
  if (ai && bi) {
    tokens += 3 * ai + bi;
  }
