...
function getAntinodes(s1, s2) {
  ...
  dy = s2y - s1y;
  result = [];
  result.push(key(s1x, s1y));
  result.push(key(s2x, s2y));

  // up
  let upx = s1x - dx;
  let upy = s1y - dy;
  while (data[upx] && data[upx][upy]) {
    result.push(key(upx, upy));
    upx -= dx;
    upy -= dy;
  }
  // down
  let downx = s2x + dx;
  let downy = s2y + dy;
  while (data[downx] && data[downx][downy]) {
    result.push(key(downx, downy));
    downx += dx;
    downy += dy;
  }
